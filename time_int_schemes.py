...
def halfexp_euler_smarminex_split(Mc,Ac,BTc,Bc,fvbc,fpbc,vp_init,B2BubBool,PrP,TsP):
	...
	IterAqq = sps.hstack([Mc,-dt*Bcc.T])
	IterAp = sps.hstack([-dt*Bcc,sps.csr_matrix((Np-1,Np-1))])
		...
		for i in range(Nts/TsP.SampInt):
			...
			if TsP.linatol == 0:
				...
				q1_tq2_p_q2_new = krypy.linsys.gmres(IterA, Iterrhs,
						x0=qqpq_old, Ml=Ml, Mr=Mr, tol=TsP.linatol, max_restarts=0, maxiter=300)#,restart=20)
