...
#
#rhs3 = 0 
rhs1p = 40*nu*pow(x,2)*pow(y,3)*sin(t) - 60*nu*pow(x,2)*pow(y,2)*sin(t) + 24*nu*pow(x,2)*y*pow((x - 1),2)*sin(t) + 20*nu*pow(x,2)*y*sin(t) - 12*nu*pow(x,2)*pow((x - 1),2)*sin(t) - 32*nu*x*pow(y,3)*sin(t) + 48*nu*x*pow(y,2)*sin(t) - 16*nu*x*y*sin(t) + 8*nu*pow(y,3)*pow((x - 1),2)*sin(t) - 12*nu*pow(y,2)*pow((x - 1),2)*sin(t) + 4*nu*y*pow((x - 1),2)*sin(t) - 4*pow(x,3)*pow(y,2)*pow((x - 1),3)*(2*x - 1)*pow((y - 1),2)*(2*y*(y - 1) + y*(2*y - 1) + (y - 1)*(2*y - 1) - 2*pow((2*y - 1),2))*pow(sin(t),2) - 4*pow(x,2)*pow(y,3)*pow((x - 1),2)*cos(t) + 6*pow(x,2)*pow(y,2)*pow((x - 1),2)*cos(t) - 2*pow(x,2)*y*pow((x - 1),2)*cos(t) + 2*x*pow(y,2)*sin(t) - 2*x*y*sin(t) - pow(y,2)*sin(t) + y*sin(t) 
#
rhs2p = -40*nu*pow(x,3)*pow(y,2)*sin(t) + 32*nu*pow(x,3)*y*sin(t) - 8*nu*pow(x,3)*pow((y - 1),2)*sin(t) + 60*nu*pow(x,2)*pow(y,2)*sin(t) - 48*nu*pow(x,2)*y*sin(t) + 12*nu*pow(x,2)*pow((y - 1),2)*sin(t) - 24*nu*x*pow(y,2)*pow((y - 1),2)*sin(t) - 20*nu*x*pow(y,2)*sin(t) + 16*nu*x*y*sin(t) - 4*nu*x*pow((y - 1),2)*sin(t) + 12*nu*pow(y,2)*pow((y - 1),2)*sin(t) + 4*pow(x,3)*pow(y,2)*pow((y - 1),2)*cos(t) - 4*pow(x,2)*pow(y,3)*pow((x - 1),2)*pow((y - 1),3)*(2*y - 1)*(2*x*(x - 1) + x*(2*x - 1) + (x - 1)*(2*x - 1) - 2*pow((2*x - 1),2))*pow(sin(t),2) - 6*pow(x,2)*pow(y,2)*pow((y - 1),2)*cos(t) + 2*pow(x,2)*y*sin(t) - pow(x,2)*sin(t) + 2*x*pow(y,2)*pow((y - 1),2)*cos(t) - 2*x*y*sin(t) + x*sin(t) 


# check the replacement ** by pow
Subdict = {x:0.1 , y:0.51, nu:3, t:5 }
